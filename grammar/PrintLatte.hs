...
instance Print (TopDef a) where
  prt i e = case e of
    ...
    ClsDef _ id clsext clsbody -> prPrec i 0 (concatD [doc (showString "class"), prt 0 id, prt 0 clsext, prt 0 clsbody])
...
instance Print (ClsExt a) where
  prt i e = case e of
    Ext _ id -> prPrec i 0 (concatD [doc (showString "extends"), prt 0 id])
    NoExt _ -> prPrec i 0 (concatD [])

instance Print (ClsBody a) where
  prt i e = case e of
    ClsBody _ clsstmts -> prPrec i 0 (concatD [doc (showString "{"), prt 0 clsstmts, doc (showString "}")])

instance Print (ClsStmt a) where
  prt i e = case e of
    MetDef _ type_ id args block -> prPrec i 0 (concatD [prt 0 type_, prt 0 id, doc (showString "("), prt 0 args, doc (showString ")"), prt 0 block])
    AttrDef _ type_ items -> prPrec i 0 (concatD [prt 0 type_, prt 0 items, doc (showString ";")])
  prtList _ [] = (concatD [])
  prtList _ (x:xs) = (concatD [prt 0 x, prt 0 xs])
...
instance Print (Stmt a) where
  prt i e = case e of
    ...
    AttrAss _ id mems expr -> prPrec i 0 (concatD [prt 0 id, prt 0 mems, doc (showString "="), prt 0 expr, doc (showString ";")])
    Incr _ id -> prPrec i 0 (concatD [prt 0 id, doc (showString "++"), doc (showString ";")])
    AttrIncr _ id mems -> prPrec i 0 (concatD [prt 0 id, prt 0 mems, doc (showString "++"), doc (showString ";")])
    Decr _ id -> prPrec i 0 (concatD [prt 0 id, doc (showString "--"), doc (showString ";")])
    AttrDecr _ id mems -> prPrec i 0 (concatD [prt 0 id, prt 0 mems, doc (showString "--"), doc (showString ";")])
...
instance Print (Type a) where
  ...
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, doc (showString ","), prt 0 xs])
instance Print (Mem a) where
  prt i e = case e of
    MemVar _ id -> prPrec i 0 (concatD [doc (showString "."), prt 0 id])
    MemInd _ expr -> prPrec i 0 (concatD [doc (showString "["), prt 0 expr, doc (showString "]")])
    MemMet _ id exprs -> prPrec i 0 (concatD [doc (showString "."), prt 0 id, doc (showString "("), prt 0 exprs, doc (showString ")")])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, prt 0 xs])
instance Print (Expr a) where
  prt i e = case e of
    ...
    ELitNull _ expr -> prPrec i 8 (concatD [doc (showString "("), prt 0 expr, doc (showString ")"), doc (showString "null")])
    ELitArr _ type_ expr -> prPrec i 7 (concatD [doc (showString "new"), prt 0 type_, doc (showString "["), prt 0 expr, doc (showString "]")])
    ELitCls _ type_ -> prPrec i 7 (concatD [doc (showString "new"), prt 0 type_])
    EExprAttr _ expr mems -> prPrec i 7 (concatD [prt 8 expr, prt 0 mems])
    ENameAttr _ id mems -> prPrec i 7 (concatD [prt 0 id, prt 0 mems])
